...
class BertData():
    def __init__(self, args):
        ...
        self.tgt_bos_token = '[unused0]'    # Beginning of sentence for target
        self.tgt_eos_token = '[unused1]'    # End of sentence for target
        self.tgt_ss_token = '[unused2]'     # Sentence separator for target

        logger.info("Using {} tokenizer".format(args.pretrained_model))
        if args.pretrained_model == 'bert':
            ...
            self.tokenizer.additional_special_tokens = self.tokenizer.additional_special_tokens \
                                                     + [self.tgt_bos_token, self.tgt_eos_token, self.tgt_ss_token]
    ...
    def preprocess(self, src, tgt, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        # vvv This : for each SEP, give the number of tokens in the sentence (including SEP)
        ...
        if self.args.pretrained_model == 'bert':
            tgt_subtokens_str = self.tgt_bos_token + ' ' + " {} ".format(self.tgt_ss_token).join(
                [' '.join(self.tokenizer.tokenize(' '.join(tt), use_bert_basic_tokenizer=use_bert_basic_tokenizer)) for tt in tgt]) \
                + ' ' + self.tgt_eos_token
        elif self.args.pretrained_model == 'xlnet':
            tgt_subtokens_str = self.tgt_bos_token + ' ' + " {} ".format(self.tgt_ss_token).join(
